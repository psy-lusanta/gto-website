...
const contactInfo = [
  {
    icon: <MapPin size={20} />,
    label: "Address",
    value: "#12 Kabignayan St., Doña Josefa, Quezon City",
  ...
  },
  {
    ...
    label: "Phone",
    value: "(02) 8-7118005",
    sub: "09688996697 | 09177146697",
  },
  {
    icon: <Mail size={20} />,
    label: "Email",
    value: "info@gtotrading.com.ph",
...
export default function Contact() {
  ...
  return (
    <section id="contact" className="py-24 bg-white/[.02]">
      <div className="max-w-6xl mx-auto px-5">
        ...
        <div className="text-center mb-16 reveal">
          ...
          <h2 className="font-black text-5xl sm:text-6xl lg:text-7xl text-white leading-none mb-4"
            style={{ fontFamily: "'Barlow Condensed', sans-serif" }}>
            Contact <em className="text-orange-500 not-italic">GTO Trading</em>
          </h2>
          <p className="text-slate-400 max-w-xl mx-auto leading-relaxed">
            Have a question about our products, pricing, or becoming a reseller?
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-16">
          {contactInfo.map((c, i) => (
            <div key={c.label}
              className="reveal bg-white/[.03] border border-white/10 rounded-2xl p-5 hover:border-orange-600/40 hover:-translate-y-1 transition-all duration-300 group"
              style={{ transitionDelay: `${i * 0.08}s` }}>
              <div className="w-10 h-10 rounded-xl bg-orange-600/15 border border-orange-600/20 flex items-center justify-center text-orange-400 mb-4 group-hover:bg-orange-600/25 transition-colors">
                {c.icon}
              </div>
              <p className="text-xs font-bold tracking-widest uppercase text-slate-500 mb-1">{c.label}</p>
              <p className="text-white text-sm font-medium leading-snug">{c.value}</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          ...
          {/* Form */}
          <div className="reveal bg-white/[.03] border border-white/10 rounded-3xl p-8">
            {submitted ? (
              ...
              <>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <input name="subject" value={form.subject} onChange={handleChange}
                    ...
                    className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm placeholder:text-slate-500 outline-none focus:border-orange-500 transition-colors" />

                  <textarea name="message" value={form.message} onChange={handleChange} required rows={5}
                    placeholder="Your message..."
                    className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm placeholder:text-slate-500 outline-none focus:border-orange-500 transition-colors resize-none" />
          ...
          <div className="reveal-right flex flex-col gap-5">
            ...
            <div className="rounded-2xl overflow-hidden border border-white/10 flex-1 min-h-64">
              <iframe
                ...
                title="GTO Trading Location"
              />
            </div>

            {/* Social + quick links */}
            <div className="bg-white/[.03] border border-white/10 rounded-2xl p-6">
              <p className="text-xs font-bold tracking-widest uppercase text-slate-500 mb-4">Follow Us</p>
              <div className="flex flex-wrap gap-3">
                <a href="https://www.facebook.com/GTOPortalPhilippines" target="_blank" rel="noreferrer"
                  className="flex items-center gap-2 px-4 py-2.5 bg-white/5 border border-white/10 rounded-xl text-slate-300 text-sm hover:border-orange-500 hover:text-orange-400 transition-all no-underline">
                  Facebook
                </a>
                <a href="#"
                  className="flex items-center gap-2 px-4 py-2.5 bg-white/5 border border-white/10 rounded-xl text-slate-300 text-sm hover:border-orange-500 hover:text-orange-400 transition-all no-underline">
